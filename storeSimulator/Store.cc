...
void Store::findAvailableSLoc(StoreLocation** sloc){    
    for (int i = 0; i < SLOC_CAPACITY; i++){
        if (storeArr[i].isAvailable())
        {
            *sloc = &storeArr[i];
            return;
        }
...
void Store::findAvailableWHLoc(WHLocation** wloc){  
    for (int i = 0; i < WHLOC_CAPACITY; i++){
...
void Store::receiveProduct(const string& pname, int quantity){
    ...
    while(quantity > 0){
        ...
        if (quantity > bloc->getCapacity()){
            ...
            //cout << endl << "recieveProduct "<< quantity;
        }
        else
        {
            //cout << endl << "recieveProduct "<< quantity;
    ...
    cout<<endl<<"stocking store location..."<<endl;
